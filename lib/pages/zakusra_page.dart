...
class ZakPage extends StatefulWidget {
  const ZakPage({Key? key}) : super(key: key);
  @override
  State<ZakPage> createState() => _ZakPageState();
}
class _ZakPageState extends State<ZakPage> {
  String selectedRecipe = 'Кексы';
  List<Recipe_Data> dataRecipe = [];

  Future<void> getRecipe() async {
    final String response = await rootBoundle.rootBundle.loadString('jsonfile/zacuska.json');
    final data = json.decode(response);
    setState(() {
      for (var element in data['zakuska']) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('Рецепты закусок',style: TextStyle(
              fontSize: 30,
              color: Colors.white,
