...
class _DesPageState extends State<DesPage> {
  ...
  Future<void> getRecipe() async {
    final String response = await rootBoundle.rootBundle.loadString('jsonfile/desert.json');
    final data = json.decode(response);
    setState(() {
      for (var element in data['desert']) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('Рецепты десертов',style: TextStyle(
              fontSize: 30,
              color: Colors.white,
