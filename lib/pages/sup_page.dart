...
class SupPage extends StatefulWidget {
  ...
  @override
  State<SupPage> createState() => _SupPageState();
}
class _SupPageState extends State<SupPage> {
  ...
  Future<void> getRecipe() async {
    final String response = await rootBoundle.rootBundle.loadString('jsonfile/sup.json');
    final data = json.decode(response);
    setState(() {
      for (var element in data['sup']) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: Text('Рецепты супов',style: TextStyle(
              fontSize: 30,
              color: Colors.white,
